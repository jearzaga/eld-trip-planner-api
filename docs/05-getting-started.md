...
## 4. Local checks before each PR (no CI)

There are no CI pipelines by decision (`03-implementation-plan.md` *Decision log*). Before each PR run ruff, the full
`uv run pytest` (unit, api, acceptance and the `tests/contract` drift checks) and re-export `requirements.txt`:
...
4. Leave **Auto-Deploy** on commit (there is no CI to gate it).
